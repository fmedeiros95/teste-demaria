					...
					<table class="data-table dt-responsive table table-borderles nowrap table-centered m-0 w-100">
						<thead>
							<tr class="table-light">
								<th>{{ __('Vendedor') }}</th>
								<th class="text-center">{{ __('Vendas') }}</th>
								<th class="text-center">{{ __('Produtos') }}</th>
								<th class="text-center">{{ __('Receita') }}</th>
							</tr>
						</thead>
					</table>
				</div>
    ...
    <script type="text/javascript">
        $(function () {
            const table = $('.data-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ url('panel/reports/sellers') }}",
                language: {
					paginate: {
                ...
                columnDefs: [{
					...
					render: (data, type, row) => {
						return parseInt(row.sales_count).toLocaleString('pt-br');
					}
				}, {
					targets: 2,
					render: (data, type, row) => {
					...
					targets: 3,
					render: (data, type, row) => {
						return parseFloat(row.total).toLocaleString('pt-br', { style: 'currency', currency: 'BRL' });
					}
				}],
