					...
					<div class="header-title d-flex w-100 justify-content-between">
						<div>
							...
							<p class="text-muted font-13 mb-4">
								{{ __('Aqui você pode ver todos os usuários cadastrados no sistema.') }}
							</p>
						</div>
						<div class="float-end">
							<button type="button" class="add-user btn btn--md btn-primary waves-effect waves-light">
								{{ __('Novo') }}
							</button>
						</div>
					</div>
					...
					<table class="data-table dt-responsive table table-borderles nowrap table-centered m-0 w-100">
                        <thead>
                            <tr class="table-light">
								<th>{{ __('Nome') }}</th>
								<th>{{ __('E-mail') }}</th>
								<th class="text-center">{{ __('Cargo') }}</th>
								<th>{{ __('Criado em') }}</th>
	...
	<script type="text/javascript">
		$(function () {
			const table = $('.data-table').DataTable({
				...
				columnDefs: [{
					...
					render: (data, type, row) => {
						let role = '???';
						switch (row.role) {
							case 'admin': role = "{{ __('Administrador') }}"; break;
							case 'seller': role = "{{ __('Vendedor') }}"; break;
						}
						return `<div class="badge bg-primary text-uppercase">${role}</div>`;
					}
				}, {
					targets: 3,
					render: (data, type, row) => {
			...
			$('.data-table').on('click', '.edit', function () {
				...
				$.ajax({
					...
					success: function({ data }) {
						...
						$('.modal-title', modalUser).text('{{ __("Editar Usuário") }}');
						$('form', modalUser).attr('action', "{{ url('panel/users') }}/" + id)
							.attr('method', 'PUT');
			...
			$('.add-user').on('click', function () {
				modalUser.modal('show');
				$('.modal-title', modalUser).text('{{ __("Adicionar Usuário") }}');
				$('form', modalUser).attr('action', "{{ url('panel/users') }}")
					.attr('method', 'POST');
